<script lang="ts">
    ...
    async function loadConfig() {
        ...
        try {
            ...
            const cfg = response.config as WafConfig;
            fullBackendConfig = cfg; // Store for correct saving
            ...
            if (cfg.upstream) {
                ...
                upstreamConfig.pool_size = cfg.upstream.pool_size;
            ...
            if (cfg.api_protection) {
                config.api_enabled = cfg.api_protection.enabled;
                config.graphql_max_depth = cfg.api_protection.graphql.max_depth;
                config.graphql_max_complexity =
                    cfg.api_protection.graphql.max_complexity;
                config.graphql_introspection =
                    cfg.api_protection.graphql.introspection_enabled;
            }
...
<div class="h-full flex flex-col p-6 space-y-6 max-w-5xl mx-auto">
    ...
    >
        ...
        {:else if loadError}
            ...
            >
                <div
                    class="bg-red-500/10 border border-red-500/30 rounded-lg p-6 max-w-md text-center"
                >
                    <AlertTriangle
                        size={32}
                    ...
                    <p class="text-amber-400 text-xs mb-4">
                        ⚠️ Displayed values are defaults, NOT current config.
                        Saving will overwrite production settings.
                    </p>
                    <button
                        on:click={loadConfig}
                        class="px-4 py-2 text-sm bg-white text-black font-medium rounded-md hover:bg-gray-200 transition-colors"
                    >
                        Retry
                    </button>
                </div>
        ...
        {:else if activeTab === "upstream"}
            <div in:fade={{ duration: 200 }} class="space-y-8 max-w-xl">
                ...
                <div class="space-y-4 pt-4 border-t border-[#222]">
                    ...
                    >
                        <span class="w-2 h-2 rounded-full bg-blue-500"></span> Health
                        Check
                    </h3>
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input
                                type="text"
                                bind:value={upstreamConfig.health_check_path}
                                class="w-full bg-[#111] border border-[#333] rounded-md px-3 py-2 text-white focus:outline-none focus:border-white transition-colors"
                                placeholder="/health"
                            />
                        </div>
            ...
            <div in:fade={{ duration: 200 }} class="space-y-8 max-w-xl">
                <div class="space-y-2">
                    ...
                    <p class="text-xs text-gray-500">
                        Comma-separated list of allowed verbs. Others will be
                        blocked.
                    </p>
                </div>
        ...
        {:else if activeTab === "general"}
            <div in:fade={{ duration: 200 }} class="space-y-6 max-w-xl">
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        {#each ["Detection", "Blocking", "Off"] as mode}
                            <button
                                ...
                                on:click={() => (config.detection_mode = mode)}
                            >
                                {mode}
                            </button>
                        {/each}
                    ...
                    <p class="text-xs text-gray-500">
                        "Blocking" will actively drop requests. "Detection" logs
                        only.
                    </p>
                </div>
